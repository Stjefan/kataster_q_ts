<template>
  ...
  <q-input label="hz63" :model-value="hz63" @update:model-value="$emit(`update:hz63`, parseInt($event))" type="number"
    class="col-2" />
  <q-input label="hz125" :model-value="hz125" @update:model-value="$emit(`update:hz125`, parseInt($event))"
    type="number" class="col-2" />
  <q-input label="hz250" :model-value="hz250" @update:model-value="$emit(`update:hz250`, parseInt($event))"
    type="number" class="col-2" />
  <q-input label="hz500" :model-value="hz500" @update:model-value="$emit(`update:hz500`, parseInt($event))"
    type="number" class="col-2" />
...
export default defineComponent({
  ...
  setup(props) {
    ...
    ].concat(['hz31_5', 'hz63']).map((i, ix) => excelFieldImportFactory.build({ maps_to: i, col: props[i], name: i })))


    const myFields = ['hz31_5', 'hz63']
